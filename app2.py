...
DB_CONFIG = {
    ...
    "port":     4253,
    "user":     "nwhite42", # Updated user
    "password": "Ewbanh_42", # Updated password
    "database": "Team11",
}
...
# app.name = 'students_25.Team11.web_application3.app2' # Example if needed
...
def api_create_model():
    ...
    try:
        ...
        # --- Determine save subdirectory based on extension ---
        if main_ext == ".xml":
            main_file_subdir = 'xml_files' # Put XMLs in xml_files subdir
        elif main_ext == ".tsv":
            # Decide where main TSVs go. Let's use 'main_tsv_files' subdir for clarity
            main_file_subdir = 'main_tsv_files'
        else:
             # Fallback - should not happen if ALLOWED_EXTENSIONS is checked
             main_file_subdir = None # Save to root

        if main_file_subdir:
            # Path relative to UPLOAD_FOLDER for DB/URL generation (use forward slashes)
            ...
            # Ensure subdirectory exists (BEFORE checking if file exists there)
            ...
            # Save to root if no subdir determined
        ...
        # Check existence AT THE FINAL DESTINATION
        if main_file_dest.exists():
            return jsonify(error=f"Main file '{main_filename}' already exists at '{main_file_relative_path}'. Upload cancelled."), 409
        ...
        try:
            ...
            app.logger.error(f"Error saving main file {main_filename} to {main_file_dest}: {save_e}", exc_info=True)
            raise IOError(f"Failed to save main model file: {save_e}") # Raise specific error
        ...
        for input_name, config in optional_files_config.items():
            ...
            if input_name in request.files and request.files[input_name].filename:
                ...
                try:
                    ...
                    app.logger.info(f"Optional file '{opt_filename}' saved successfully to {opt_dest}")
                    ...
                    # Log error but continue processing other files. Path remains None for this one.
            else:
                 # Log if file input name is not in request.files OR if filename is empty
                 if input_name in request.files:
                      app.logger.debug(f"Empty filename provided for optional input: {input_name}")
                 else:
                      app.logger.debug(f"No file provided in request for optional input: {input_name}")
        ...
        # file_link now stores the relative path of the main file
        meta = {
            ...
            "Biomass_RCH1":      None, # Column seems unused now
        }
        ...
        try:
            ...
            conn_local.commit() # Commit transaction
            ...
            response_meta = {
                 ...
                 # "file_link_path": meta.get("file_link"), # Client probably doesn't need this raw path
    ...
    # --- Outer Exception Handler ---
        ...
        elif isinstance(e, mariadb.Error): # Catch other specific DB errors
        ...
        elif isinstance(e, mariadb.OperationalError):# Specific check for connection errors
        ...
        # Note: 'cur' might not be defined if get_db_cursor() failed early in the 'try' block
...
if __name__ == "__main__":
    # Example: python app2.py (or whatever you name this file)
